...
----------------------------------------------------*/
	...
	function CustomFunction() {
		...
		document.querySelectorAll("textarea").forEach(textarea => {
			// Create a counter element for this textarea
			...
			textarea.addEventListener("focus", () => {
				counter.style.display = "block"; // Show on focus
			});
		
			textarea.addEventListener("blur", () => {
				counter.style.display = "none"; // Hide on blur
			});
			...
			textarea.addEventListener("input", () => {
				...
				counter.textContent = `Word count: ${words.length} / ${maxWords}`;
			});
		});
		...
		gsap.from('.foreward-section span', {
			scrollTrigger: {
				trigger: '.foreward-section',
				start: '-30% 50%',  // Start the animation when the top of the element is at 80% of the viewport
				end: 'bottom 80%',    // Adjust end position as needed
		...
		if(window.innerWidth > 737){
			campaignTl.to(".flip", {
				rotateY: -180,
				ease: "ease.inOut",
				onStart: () => gsap.set(".flip", { pointerEvents: "none" }),
				onReverseComplete: () => gsap.set(".flip", { pointerEvents: "auto" })
			},'flip')
		}
		else{	
			campaignTl.to(".flip, .page-right", {
				// rotateY: -180,
				left: '0',
			...
			campaignTl.to(".second-front-mob", {
				margin: '0',
				ease: "ease.inOut"
			},'flip')
			campaignTl.to(".last-page-mob", {
				left: '0',
		...
		campaignTl.to(".img-magnifier-glass", {
			width: 0,
			height: 0,
			opacity: 0,
			border: 'none',
		...
		campaignTl.to(".page-right-flip", {
			rotateY: -180,
			ease: "ease.inOut"
		})
		campaignTl.to(".flip .back", {
			opacity: '0',
			ease: "ease.inOut"
		})

		if (document.querySelector('.flip')) {
			let hoverItem = document.querySelector('.flip');
			hoverItem.addEventListener("mouseover", () =>
			{
				console.log('hovered')
				gsap.to('.light-content #ball',{
					border:'none',
					ease: "ease.inOut"
				})
				gsap.to(".img-magnifier-glass", {
					width: '100px',
					height: '100px',
					opacity: 1,
...
---------------------------------------------------*/	
	...
	function ShowcaseGallery() {
		...
		if( $('.showcase-gallery').length > 0 ){
			...
			$('.trigger-item').on('click', function() {
				// if (!(document.body.classList.contains('index-page-body'))) {
					// console.log('hdj')
					$("body").addClass("load-project-thumb");
		...
		if ($("body").hasClass("index-page-body")) {
			const audio1  = document.querySelector('#header_audio_section-2')
			gsap.to($('.index-page-body .hero-video-wrapper video'), {
				scrollTrigger: {
				  trigger: ".index-page-body .hero-video-wrapper",
				  start: "top center", // When top of the video container hits the center of the viewport
				//   markers:true,
				  onEnter: () => {
					raiseVolume('myVideo','video')
					reduceVolume('header_audio_section-2')
					$('.index-page-body .hero-video-wrapper video').get(0).play(); // Play the video
				  },
				  onLeave: function() {
					raiseVolume('header_audio_section-2','audio')
					reduceVolume('myVideo');
					setTimeout(() => {
						$('.index-page-body .hero-video-wrapper video').get(0).pause()
					}, 2000);
				},
		...
		if (window.location.pathname === '/index.html' || window.location.pathname === '' || window.location.pathname === '/' ) {
			// console.log('in')
			gsap.to($('.hmt-old-video video'), {
				scrollTrigger: {
				  trigger: ".hmt-old-video",
				  start:  window.innerWidth < 737 ? "150% bottom" :  "130% center", // When top of the video container hits the center of the viewport
				  end: window.innerWidth < 737 ? "350% 60%" : "240% 60%",
				//   markers:true,
				  onEnter: () => {
					reduceVolume('header_audio_section-2');
					document.getElementById("hmt_video").play();
					raiseVolume('hmt_video','video');
				  },
				  onLeave: function() {
					reduceVolume('hmt_video');
					raiseVolume('header_audio_section-2','audio')
					document.getElementById("hmt_video").pause();
				},
				  onEnterBack: function() {
					reduceVolume('header_audio_section-2');
					document.getElementById("hmt_video").play();
			  ...
			  //    watch 3d js start
			// gsap.set(".watch-3d-img", { rotationY: -20 });

			// Animation based on scroll
			gsap.to(".watch-3d-img", {
				rotationZ: 40,
				scale: 1.6,
				delay: 0.6,
				scrollTrigger: {
					trigger: ".watch-3d-outer",
					start: "top 80%",
					end: "bottom top",
					scrub: true,
					// markers: true,
		...
		const windows_width = window.innerWidth < 737 ? '450%' : '550%' 
		const windows_scroll = window.innerWidth < 737 ? '-620%' : '-300%' 
        const tl5 = gsap.timeline({
            scrollTrigger: {
            trigger: ".parent",
            start: "0% top",
            end: `${windows_width} center`,
            pin: true,
            scrub: 1,
            },
        });

      tl5.to(".overview-section", { x: `${windows_scroll}` });

	if (window.location.pathname === '/' || window.location.pathname.endsWith('index.html')) {
		let sections = gsap.utils.toArray(".panel");

		gsap.to(sections, {
			xPercent: (window.innerWidth < 737 ? -300 : -100) * (sections.length - 1),
			ease: "none",
			scrollTrigger: {
			trigger: ".container",
			pin: true,
			scrub: 1,
			// markers:true,
			snap: window.innerWidth < 737 ? false :1 / ((sections.length - 1)*  (window.innerWidth < 737 ? 4 : 1)) ,
			end: () => "+=" + document.querySelector(".container").offsetWidth + window.innerWidth < 737 ? 2 : 0
			}
		});
	}
	  ...
	  let arrow = document.querySelector('.arrow');
		...
		// js for cursor image
		// gsap.set('.cursor-image img.cursor-hov', { yPercent: -50, xPercent: -50 })

		// gsap.utils.toArray(".cursor-image").forEach((el) => {
		// const image = el.querySelector('img.cursor-hov'),
		...
		//   const pinSection = document.querySelector(".what-killed-hmt-text");
		//   gsap.to(pinSection, {
		// 	scrollTrigger: {
		// 	  trigger: ".dark-section-container",
		// 	  start: "top top", // When top of the parent div hits the top of the viewport
		// 	//   endTrigger: ".content", // End trigger when the content div is reached
		// 	  end: "bottom bottom", // End pinning when bottom of the content div hits the bottom of the viewport
		// 	  pin: true, // Pin the section
		// 	  markers:true,
		// 	//   pinSpacing: false // Keep the pinSection at the top of the viewport
		// 	}
		//   });
	...
	function isIndexPage() {
		// Check if the current URL path is '/' or '/index.html'
	...
	// gsap.from(".fullpage-title", {
	// 	y: -50,  // Starting position below the final position
	// 	opacity: 0, // Start with it invisible
	// 	scrollTrigger: {
	// 		trigger: ".scrollable-setion",
	// 		start: "100% left",  // When the scrollable section is at the start of the viewport
	// 		end: "200% top", // End when the section scrolls out of the viewport
...
if(window.location.pathname == '/contact.html'){
	...
	function ContactFormSubmit(){
		...
		radio.addEventListener('change', () => {
			...
			otherInput.required = true; // Make input required when "Other" is selected
			} else {
			otherInputWrapper.style.display = 'none';
			otherInput.value = '';      // Clear the field
		...
		function validateEmail(email) {
			...
			if (blockedDomains.includes(domain)) {
				emailError.textContent = 'Please use your company email address';
				emailError.style.display = 'block';
				return false;
			}
		...
		function updateSubmitButton() {
			// const isValid = validateName(nameInput.value) && validateEmail(emailInput.value);
		...
		? 'http://localhost:3000'
		: 'https://nyuct-node-server.vercel.app';
	
		const getAccessEndpoint = isLocalhost
		? '/full-access'          // your local endpoint path
		: '/api/full-access';    // production endpoint path
		...
		const textarea = textInput.value.trim();
		const sector = form.querySelector('input[name="sector"]:checked')?.value;
		const subject = form.querySelector('input[name="interest"]:checked')?.value;
		
		// If "Other" is selected, use the text field value instead
		const finalSubject = subject === 'other' ? otherInput.value.trim() : subject;
		...
		try {
			const response = await fetch(`${backendBaseURL}${getAccessEndpoint}`, {
			method: 'POST',
	...
	// updateSubmitButton();
// Functions to execute for loading via ajax 
function AjaxScrollCustom(){
	const ScrollArea = document.querySelector('#content-scroll');

		// Optional: Check if ScrollArea is defined
		if (ScrollArea) {
			// Initialize the scrollbar
			const scrollbar = Scrollbar.init(ScrollArea, {
			});

			// Add scroll event listener to the scrollbar instance
			scrollbar.addListener((status) => {
				// console.log('Scroll position after scrollbar initialization:', status.offset.y);
				// if((window.location.pathname == '/' || window.location.pathname == '/index.html' || window.location.pathname == '/about.html' || window.location.pathname == '/contact.html' || window.location.pathname == '/about-full.html' || window.location.pathname == '/project01.html') ){
		...
		class HoverImgFx1 {
			...
			initEvents() {
				...
				this.positionElement = (ev) => {
					...
					if ($("body").hasClass("scroll-hover")) {
						// console.log('it is inside')
						const docScrolls = {
							left : document.body.scrollLeft + document.documentElement.scrollLeft, 
							top : - scrollbar.scrollTop
						};
						gsap.to($('.hover-reveal'), { duration: 0.7, top: `${mousePos.y-(this.DOM.el.querySelector('.hover-reveal').offsetHeight*0.2)-docScrolls.top}px`, left: `${mousePos.x-(this.DOM.el.querySelector('.hover-reveal').offsetWidth*0.2)-docScrolls.left}px`, ease:Power4.easeOut });
					} else {
						const docScrolls = {
							left : document.body.scrollLeft + document.documentElement.scrollLeft, 
						...
						// condition for article:nth-child(4n+3) child element for hover reveal image in timeline
						if(ev.target.classList.contains("bottom-article")){
							console.log('inside')
							gsap.to($('.hover-reveal'), { duration: 1, top: `${mousePos.y-300-docScrolls.top}px`, left: `${mousePos.x + 100-docScrolls.left}px`, ease:Power4.easeOut });
						}else{
							console.log('outside')
							gsap.to($('.hover-reveal'), { duration: 1, top: `${mousePos.y+10-docScrolls.top}px`, left: `${mousePos.x+10-docScrolls.left}px`, ease:Power4.easeOut });
						}
					}
...
// // Disable common DevTools key combos
