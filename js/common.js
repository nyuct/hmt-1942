...
---------------------------------------------------*/
	...
	window.ScrollEffects = function() {
		...
		$(".video-section-outer iframe").on("mouseenter", function() {
			$('#ball').addClass('d-none');
			// console.log("entered");
		});
		
		$(".video-section-outer iframe").on("mouseleave", function() {
		...
		const timeline = gsap.timeline({
			scrollTrigger: {
				trigger: "#myVideo", // The element that triggers the animation
				start: "center center", // When the top of the element hits the center of the viewport
				end: "200% center", // When the bottom of the element hits the center of the viewport
				toggleActions: "play none none reverse", // play, pause, resume, reverse
				scrub:2,
				pin:'.video-section-outer',
				// markers: true, // Helpful for debugging
			}
		});	
		timeline.from(".video-section-outer", {
		...
		const watchTimeline = gsap.timeline({
			scrollTrigger: {
				trigger: ".watch-3d-outer", // The element that triggers the animation
				start: window.innerWidth < 768 ? "top 16%" : "bottom 100%", // When the top of the element hits the center of the viewport
				end: "120% center", // When the bottom of the element hits the center of the viewport
				toggleActions: "play none none reverse", // play, pause, resume, reverse
				scrub:2,
				pin:'.watch-3d-outer',
				// markers: true, // Helpful for debugging
		...
		watchTimeline.to(".watch-3d-outer", {
			opacity:0,
			ease: "power4.out"
		})		

		const nearDeath = gsap.timeline({
			scrollTrigger: {
				trigger: ".near-death-wrapper", // The element that triggers the animation
				start: "top 30%", // When the top of the element hits the center of the viewport
				end: "80% center", // When the bottom of the element hits the center of the viewport
				toggleActions: "play none none reverse", // play, pause, resume, reverse
				scrub:2,
				pin:'.near-death-wrapper',
				// markers: true, // Helpful for debugging
			}
		});		
		nearDeath.from(".the-near-death", {
			opacity:0,
			ease: "power4.out"
		},'together')		

		const nearDeath2 = gsap.timeline({
			scrollTrigger: {
				trigger: ".near-death-wrapper", // The element that triggers the animation
				start: "top 20%", // When the top of the element hits the center of the viewport
				end: "160% center", // When the bottom of the element hits the center of the viewport
				toggleActions: "play none none reverse", // play, pause, resume, reverse
				scrub:2,
				// markers: true, // Helpful for debugging
			}
		});	
		nearDeath2.from(".crime-section", {
			opacity:0,
			ease: "power4.out"
		})		
				...
				const rTimeline = gsap.timeline({
					scrollTrigger: {
						...
						start: function() {
							...
							const endPin =  window.innerHeight;
							return "+=" + endPin;
						},
						pin: true,
						scrub: true,
				...
				gsap.to(listRotatorTitle, {
					scrollTrigger: {
						...
						start: function() {
							...
							const endPin = listRotator.offsetHeight * 1;
							return "+=" + endPin;
						},
						pin:true,
						scrub: true,
		...
		contentVideo.forEach(function(videoPlay) {
			...
			var videoScene = ScrollTrigger.create({
				...
				start: "700% 800%",
				// markers:true,
				end: () => `+=${videoPlay.offsetHeight + window.innerHeight*5}`,
				onEnter: function() {
					 video.play();
				},
				onLeave: function() {
		...
		if ($(".content-row").hasClass("light-section")) {
			$(".light-section").each(function(i) {				
				$(this).wrap( "<div class='light-section-wrapper mb-20'><div class='light-section-container content-max-width'></div></div>" );
		...
		jQuery(document).ready(function(){
			  ...
			  $(window).on('popstate', function() {
				...
				if(window.location.pathname ==  '/index.html') {
					document.querySelector('#header_audio_section-2').pause()
				}

				isAnimating = true;
				// trigger page animation
				if(url == 'about.html') {
					$('body').removeClass('index-page-body');
					$('body').addClass('our-vision');
					$('body').addClass('ajax-about-page');
					if($('body').hasClass('scroll-hover2')){
						$('body').removeClass('scroll-hover2');
						$('body').addClass('scroll-hover');
					}
					console.log('1')
				}
				if(url == 'index.html') {
					$('body').addClass('index-page-body');
					$('body').removeClass('our-vision');
					console.log('2')
				}
				if(url !==  'index.html') {
					$('body').removeClass('index-page-body');
				...
				LoadViaAjax();
				if(url == 'index.html') {
					const ajaxAudio  = document.querySelector('#header_audio_section-2')
					ajaxAudio.volume = 0.3;
					ajaxAudio.play();
					const youtubeVid = document.getElementById('myVideo');
					const hmtVid = document.getElementById('hmt_video');
					youtubeVid.muted = false; 
					hmtVid.muted = false; 
					$('.hero-video-wrapper').css('opacity','1')
					ScrollTrigger.create({
						trigger: ".team-members-list", // Change this to your section's class or ID
						start: "-50% center",
						end: "130% center",
						// markers:true,
						onLeave: () => {
							console.log('in')
							document.body.classList.remove("scroll-hover")
							document.body.classList.add("scroll-hover2")
						},
						onEnterBack: () => {
							document.body.classList.remove("scroll-hover2")
							document.body.classList.add("scroll-hover")
						}
					});
				}
				if(url== 'contact.html'){
				...
				gsap.to($(".hero-subtitle span"), {duration: 0.5, opacity:1, x:0, ease:Power2.easeOut});
				if (!isMobile() && !$('body').hasClass("disable-cursor")) {
					$(".sticky.left").mouseenter(function(e) {
						var rcBounds = $(this)[0].getBoundingClientRect();		  
						var positionX = rcBounds.left - offsetX;
					...
					$('.dark-section-wrapper').removeClass('mt-20')
				}
				...
				//   $('.dark-section-wrapper').removeClass('mt-20')
				  ...
				  if(url!=window.location && bool){
					...
					// console.log('2')
					if(window.location.pathname == '/project01.html' || window.location.pathname == '/project02.html' || window.location.pathname == '/project03.html' || window.location.pathname == '/janata.html' || window.location.pathname == '/1942.html'){
						$('link[href*="common-style.css"]').remove();
					}
					if(!(window.location.pathname == '/project01.html' || window.location.pathname == '/project02.html' || window.location.pathname == '/project03.html' || window.location.pathname == '/janata.html' || window.location.pathname == '/1942.html')){
						  if (!($('link[href*="common-style.css"]').length > 0)) {
							$('<link>', {
								rel: 'stylesheet',
								type: 'text/css',
